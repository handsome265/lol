...
class StickmanPlayer {
  ...
  createStickman() {
    ...
    this.leftArm.position.set(-0.2, 1.2, 0);
    ...
    this.rightArm.position.set(0.2, 1.2, 0);
    ...
    this.leftLeg.position.set(-0.1, 0.7, 0);
    ...
    this.rightLeg.position.set(0.1, 0.7, 0);
    this.group.add(this.rightLeg);

    this.scene.add(this.group);
    this.mesh = this.group; // 兼容性
  }
  ...
  updateWalkAnimation(delta) {
    ...
    if (this.isWalking) {
      ...
      this.head.position.y = 1.5 + Math.abs(Math.sin(this.walkCycle * 2)) * 0.05;
    } else {
      // 恢復待機姿勢
